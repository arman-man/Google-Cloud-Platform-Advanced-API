...
const { BOAT, USER, APP_URL } = require('./constants');
...
async function boat_update_name_constraint(name) {
    ...
    const allBoats = await get_boats();
    ...
    get_boat(id)
        .then(boat => {
            if (boat[0] === undefined || boat[0] === null) {
                ...
                if (boat[0].owner != null && boat[0].owner != undefined) {
                    boat[0].owner["self"] = APP_URL + "/users/" + boat[0].owner.id;
                }
...
// patch boat name, type, or length
...
// create relationship between boat-load
router.put('/:bid/loads/:lid', async function (req, res) {
...
router.delete('/:id', async function (req, res) {
    ...
    if (boat[0] === undefined || boat[0] === null) {
        ...
        const handleLoadRelationships = boat[0].loads && boat[0].loads.length > 0
            ? Promise.all(boat[0].loads.map(lid => delete_relationship_boat_load(id, lid, get_load, update_load)))
            : Promise.resolve();

        // Handle the relationship with the user (if necessary)
        const handleUserRelationship = boat[0].owner
            ? delete_relationship_user_boat(boat[0].owner, id)
            : Promise.resolve();

        // Execute both actions and then delete the boat
        Promise.all([handleLoadRelationships, handleUserRelationship])
            .then(() => delete_boat(id))
            .then(() => res.status(204).end())
            .catch(error => {
...
module.exports = { router, get_boat, update_boat };
